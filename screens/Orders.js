...
export default function Orders() {
  ...
  useEffect(() => {
    getOrders()
      .then((data) => {
        const newOrders = data.flatMap((item) => item.items.flat());
        setOrder(newOrders);
        setLoading(false);
      })
      .catch((err) => {
            ...
            <ScrollView className="divide-y divide-gray-200">
              {Object.entries(orders).map(([key, items]) => {
                return (
                  ...
                  >
                    <Image
                      source={{ uri: urlFor(items?.image).url() }}
                      className="h-12 w-12"
                    />
                    <Text className="text-[#00ccbb]">{items?.count} x </Text>
                    <Text className="flex-1 font-medium">{items?.name}</Text>
                  </View>
                );
              })}
            </ScrollView>
